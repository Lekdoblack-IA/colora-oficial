...
const HowItWorksStep = ({ step, index }: { step: typeof steps[0], index: number }) => {
  ...
  const { ref: centerRef, isInCenter } = useCenterAlignment();
  ...
  return (
    ...
    >
      {/* Número */}
      <div className={`relative inline-flex items-center justify-center w-16 h-16 rounded-full bg-white border border-black mb-6 transition-all duration-300 ${
        isMobile && isInCenter 
          ? 'border-pink-500 shadow-lg -translate-y-1 bg-pink-50'
          : !isMobile 
            ? 'group-hover:border-pink-500 group-hover:shadow-lg group-hover:-translate-y-1 group-hover:bg-pink-50'
            : ''
      }`}>
      ...
      {/* Ícone */}
      <div className="mb-4">
        <Icon className={`w-12 h-12 mx-auto transition-colors duration-300 ${
          isMobile && isInCenter
            ? 'text-pink-500'
            : !isMobile
