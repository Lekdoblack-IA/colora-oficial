...
const faqItems = [{
  ...
  answer: <div className="space-y-3">
        ...
        <div className="space-y-2">
          {["Momentos especiais e memoráveis (Viagens, casamentos, passeios, encontros...)", "Formato vertical (retrato) (Se adapta perfeitamente ao livro de colorir)", "Cenário rico e visível (Paisagens, ambientes temáticos, quartos decorados...)", "Personagens inteiros no quadro (Evita cortes estranhos, melhora o resultado)", "Imagem nítida e bem iluminada (Evita distorções e traços confusos)"].map((item, index) => <div key={index} className="flex items-start space-x-2">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-3xl border border-black p-8 hover:border-transparent hover:shadow-lg transition-all duration-300">
          <Accordion type="single" collapsible className="w-full">
            {faqItems.map((item, index) => <AccordionItem key={index} value={`item-${index}`} className="border-b border-gray-200 last:border-b-0">
